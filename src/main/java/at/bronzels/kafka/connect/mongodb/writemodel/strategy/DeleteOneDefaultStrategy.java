...
public class DeleteOneDefaultStrategy implements WriteModelStrategy {
    ...
    public WriteModel<BsonDocument> createWriteModel(SinkDocument document) {
        ...
        if(idStrategy == null) {
            return kd.containsKey(at.bronzels.libcdcdw.Constants.RK_4_MONGODB_AND_OTHER_DBS_ID_FIELD_NAME)
                    ? new DeleteOneModel<>(kd)
                    : new DeleteOneModel<>(new BsonDocument(at.bronzels.libcdcdw.Constants.RK_4_MONGODB_AND_OTHER_DBS_ID_FIELD_NAME,kd));
        }
        ...
        return new DeleteOneModel<>(
                new BsonDocument(at.bronzels.libcdcdw.Constants.RK_4_MONGODB_AND_OTHER_DBS_ID_FIELD_NAME,_id)
        );
