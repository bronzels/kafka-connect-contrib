...
import at.bronzels.kafka.connect.mongodb.id.strategy.BsonOidStrategy;
import at.bronzels.kafka.connect.mongodb.id.strategy.FullKeyStrategy;
import at.bronzels.kafka.connect.mongodb.writemodel.strategy.ReplaceOneDefaultStrategy;
...
public class MongoDbSinkTaskTest {
    ...
    void testCreateSinkRecordBatchesPerTopicWithDefaultTopicAndNoBatching() {
        ...
        props.put(MongoDbSinkConnectorConfig.COLLECTION_CONF,settings.collection);
        sinkTask.start(props);

        List<SinkRecord> sinkRecords = createSinkRecordList(settings);
        Map<String, SinkRecordBatches> batchesMap = sinkTask.createSinkRecordBatchesPerTopic(sinkRecords);
    ...
    void testCreateSinkRecordBatchesPerTopicWithSingleTopicAndNoBatching() {
        ...
        Map<String,String> props = new HashMap<>();
        props.put(MongoDbSinkConnectorConfig.COLLECTIONS_CONF,settings.collection);
        props.put(MongoDbSinkConnectorConfig.COLLECTION_CONF +"."+settings.topic,settings.collection);
        props.put(MongoDbSinkConnectorConfig.MAX_BATCH_SIZE +"."+settings.collection,String.valueOf(settings.batchSize));
        sinkTask.start(props);
    ...
    Collection<DynamicTest> testCreateSinkRecordBatchesPerTopicWithMultipleTopicAndDifferentBatchSizes() {
        ...
        Map<String,String> props = new HashMap<>();
        props.put(MongoDbSinkConnectorConfig.COLLECTIONS_CONF,
                settings.stream()
                        .map(ts -> ts.collection)
                        .collect(Collectors.joining(MongoDbSinkConnectorConfig.FIELD_LIST_SPLIT_CHAR)));

        settings.forEach(ts -> {
            props.put(MongoDbSinkConnectorConfig.COLLECTION_CONF +"."+ts.topic,ts.collection);
            props.put(MongoDbSinkConnectorConfig.MAX_BATCH_SIZE +"."+ts.collection,String.valueOf(ts.batchSize));
        });
        ...
        Map<String, SinkRecordBatches> batchesMap =
        ...
        settings.forEach(ts -> {
                ...
                tests.add(dynamicTest("verify contents of created batches map for " + namespace , () -> {
                    SinkRecordBatches batches = batchesMap.get(namespace);
    ...
    void testBuildReplaceOneModelsCustomConfigSinkRecordsWithValuePresent() {
        ...
        MongoDbSinkTask sinkTask = new MongoDbSinkTask();
        Map<String,String> props = new HashMap<>();
        props.put(MongoDbSinkConnectorConfig.DOCUMENT_ID_STRATEGY_CONF, BsonOidStrategy.class.getName());
        props.put(MongoDbSinkConnectorConfig.WRITEMODEL_STRATEGY, ReplaceOneDefaultStrategy.class.getName());
        sinkTask.start(props);
                ...
                writeModelList.stream().map(wm ->
                        () -> assertAll("assertions for single WriteModel",
                                ...
                                () -> {
                                    ...
                                    assertEquals(1,filter.size(),"filter document should only contain "+at.bronzels.libcdcdw.Constants.RK_4_MONGODB_AND_OTHER_DBS_ID_FIELD_NAME);
                                    assertTrue(filter.get(at.bronzels.libcdcdw.Constants.RK_4_MONGODB_AND_OTHER_DBS_ID_FIELD_NAME).isObjectId(),"filter document _id was not of type ObjectId");
                                    assertTrue(rom.getReplacement().get(at.bronzels.libcdcdw.Constants.RK_4_MONGODB_AND_OTHER_DBS_ID_FIELD_NAME).isObjectId(),"replacement document _id was not of type ObjectId");
    ...
    void testBuildReplaceOneModelsCustomConfigSinkRecordsWithKeyAndValuePresent() {
        ...
        MongoDbSinkTask sinkTask = new MongoDbSinkTask();
        Map<String,String> props = new HashMap<>();
        props.put(MongoDbSinkConnectorConfig.DOCUMENT_ID_STRATEGY_CONF, FullKeyStrategy.class.getName());
        props.put(MongoDbSinkConnectorConfig.WRITEMODEL_STRATEGY,ReplaceOneDefaultStrategy.class.getName());
        props.put("topics","blah");
        props.put(MongoDbSinkConnectorConfig.COLLECTIONS_CONF,"blah-collection");
        props.put(MongoDbSinkConnectorConfig.COLLECTION_CONF +"."+"blah","blah-collection");
        sinkTask.start(props);
    ...
    void testBuildDeleteOneModelsCustomConfigSinkRecordsWithKeyAndNullValuePresent() {
        ...
        props.put(MongoDbSinkConnectorConfig.DOCUMENT_ID_STRATEGY_CONF,FullKeyStrategy.class.getName());
        props.put(MongoDbSinkConnectorConfig.WRITEMODEL_STRATEGY,ReplaceOneDefaultStrategy.class.getName());
        props.put(MongoDbSinkConnectorConfig.DELETE_ON_NULL_VALUES,"true");
        props.put("topics","foo");
        props.put(MongoDbSinkConnectorConfig.COLLECTIONS_CONF,"foo-collection");
        props.put(MongoDbSinkConnectorConfig.COLLECTION_CONF +"."+"foo","foo-collection");
        sinkTask.start(props);
    ...
    void testBuildWriteModelCdcForRdbmsInsert() {
        ...
        props.put(MongoDbSinkConnectorConfig.COLLECTIONS_CONF,"dbserver1.catalogA.tableB");
        props.put(MongoDbSinkConnectorConfig.COLLECTION_CONF
                        +"."+"dbserver1.catalogA.tableB","dbserver1.catalogA.tableB");
        props.put(MongoDbSinkConnectorConfig.CHANGE_DATA_CAPTURE_HANDLER
